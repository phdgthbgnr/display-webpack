...
##### npm run watch -> default format in config-banner.js
...
##### npm run dev -> default format in config-banner.js
...
- (l'export des svg par lots génére automatiquement cette nomenclature).
  ```html
  <%= htmlWebpackPlugin.options.distSVGPath %>/img/shapes_<%=
...
banner 120x600 V1
...
Si le pattern n'est pas présent, renseigner la correspondance de taille dans custom.js
(voir plus bas)
</span>**
...
**<span style="color:red">Les infos de title et éventuellement les correspondances de tailles sont à renseigner dans le fichier custom.js :</span>**
...
module.exports = {
  ...
  famille: 'Display IAB',
  png: { quality: { min: 0.6, max: 0.7 } },
  sizes: {
    'no-size': { width: 160, height: 600 },
